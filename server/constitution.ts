...
const RULES: Record<FlapRuleId, FlapConstitutionRule> = {
  "001": {
    ...
    fixGuidance: [
      ...
      "Fix Rule 001 (bucket solvency): whatever a bucket already owes must stay visible to every free-balance computation. When you credit claimable[user], either debit the bucket in the same statement or increment an aggregate (totalOwedToUsers) and subtract that aggregate everywhere availability is computed. Releasing a reservation counter at approval time without tracking the amount as owed lets the same BNB be promised twice.",
    ],
