...
import { probeCodegenFactory, type FactoryProbeResult } from "../lib/flap-factory";
import {
...
import { launchCodegenToken } from "../lib/flap-launch";
import {
  DEFAULT_LAUNCH_TAX_PERCENT,
...
export default function LaunchOnFlapPanel({
  ...
  /** Buy/sell tax as percent strings (e.g. "5" = 5% = 500 bps). Flap allows 1%–10%. */
  const [buyTaxPercent, setBuyTaxPercent] = useState(DEFAULT_LAUNCH_TAX_PERCENT);
  const [sellTaxPercent, setSellTaxPercent] = useState(DEFAULT_LAUNCH_TAX_PERCENT);
  ...
  const factoryFingerprint = configuredFactory
    ...
    : useDbFactory
      ...
      : getStoredFactoryArtifactFingerprint(targetChainId);
  ...
  }, [persistedVaultState?.factoryAddress, targetChainId]);
  ...
  useEffect(() => {
    ...
    void probeCodegenFactory(factoryAddress, targetChainId).then((result) => {
      if (!cancelled) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    setCheckingReg(true);
    void readRegisteredVault(factoryAddress, address, targetChainId)
      .then((res) => {
        if (cancelled) return;
        if (!res.registered) {
          setRegisteredKey("");
  ...
  const flapBase = flapBaseForChain(config, targetChainId);
  const explorerBase = explorerBaseForChain(config, targetChainId);
  ...
  const mainnetGateOk =
    ...
    (mainnetAckRealFunds &&
      mainnetAckReviewed &&
      mainnetAckNotAudit &&
      mainnetConfirmText.trim().toUpperCase() === tokenSymbol.trim().toUpperCase() &&
  ...
  const ensureTargetNetwork = useCallback(async () => {
    ...
    try {
      ...
      setPhase("idle");
      setError(
        err instanceof Error ? err.message : `Failed to switch to ${getFlapNetwork(targetChainId).label}.`
      );
      return false;
    }
  ...
  const deployFactory = useCallback(async () => {
    ...
    if (!(await ensureTargetNetwork())) return;
    ...
    try {
      ...
      const receipt = await waitForTransactionReceipt(wagmiConfig, { hash, chainId: targetChainId });
      const deployed = receipt.contractAddress;
      if (!deployed) throw new Error("Factory deploy did not return a contract address.");
      saveCodegenFactoryAddress(deployed, CURRENT_FACTORY_FP, targetChainId);
  ...
  const registerVault = useCallback(async () => {
    ...
    try {
      const hash = await registerVaultForFlap(
        factoryAddress,
        effectiveBytecode as `0x${string}`,
        desc,
        targetChainId
      );
      setRegisterTx(hash);
  ...
  const launchToken = useCallback(async () => {
    ...
    const buyTaxRateBps = parseTaxPercentToBps(buyTaxPercent);
    const sellTaxRateBps = parseTaxPercentToBps(sellTaxPercent);
    if (buyTaxRateBps === null || sellTaxRateBps === null) {
      setError(
        `Buy/sell tax must each be between 1% and ${MAX_PORTAL_TAX_BPS / 100}% (Flap limit).`
      );
      return;
    }
    ...
    if (isMainnetTarget && !mainnetGateOk) {
      setError("Complete the mainnet confirmation checklist before launching.");
      return;
    }
    if (!(await ensureTargetNetwork())) return;
    ...
    try {
      const result = await launchCodegenToken(launchInput, {
      ...
      const gmgn = gmgnTokenUrl(targetChainId, record.tokenAddress);
      persistLaunchStatus({
        status: "launched",
        contractName,
        wallet: address,
      ...
      void createLaunchedTokenRecord({
        ...
        vaultAddress: result.vault,
        factoryAddress,
        launchContractAddress: launchPortalForChain(targetChainId),
        registerTxHash: registerTx,
  ...
  const resetFactory = useCallback(() => {
    clearCodegenFactoryAddress(targetChainId);
  ...
  }, [contractName, persistLaunchStatus, targetChainId]);
  ...
  const buyTaxBpsPreview = parseTaxPercentToBps(buyTaxPercent);
  const sellTaxBpsPreview = parseTaxPercentToBps(sellTaxPercent);
  const launchPayloadIssue = checkLaunchPayload({
    ...
    vaultDataMode: "registered",
    buyTaxRateBps: buyTaxBpsPreview ?? undefined,
    sellTaxRateBps: sellTaxBpsPreview ?? undefined,
  });
  const taxInvalid = buyTaxBpsPreview === null || sellTaxBpsPreview === null;
    ...
    <div className="rounded-md border border-border bg-secondary/20 p-3 text-xs">
      <div className="space-y-2">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <p className="font-medium text-foreground">Launch on Flap ({targetNet.label})</p>
          <div
            role="group"
            aria-label="Launch network"
            className="flex items-center rounded-full border border-border bg-background/50 p-0.5 text-[0.65rem] font-medium"
          >
            {(
              [
                [FLAP_BSC_TESTNET.chainId, FLAP_BSC_TESTNET.shortLabel],
                [FLAP_BSC_MAINNET.chainId, FLAP_BSC_MAINNET.shortLabel],
              ] as const
            ).map(([id, label]) => (
              <button
                key={id}
                type="button"
                disabled={busy}
                onClick={() => setTargetChainId(id)}
                className={
                  targetChainId === id
                    ? "rounded-full bg-secondary px-2.5 py-0.5 text-foreground"
                    : "rounded-full px-2.5 py-0.5 text-muted-foreground transition-colors hover:text-foreground"
                }
              >
                {label}
              </button>
        ...
        <p className="text-muted-foreground">
          Deploy the factory, register your vault on-chain, then launch the token — the vault attaches
          automatically. Default is testnet; switch to mainnet only when you&apos;re ready for real BNB.
        </p>
        {isMainnetTarget ? (
          <p className="text-amber-300/90">
            Mainnet uses real BNB. Tax rates and metadata cannot be changed after launch.
          </p>
        ) : (
          <p className="text-muted-foreground/80">
            We recommend launching on testnet first to verify the vault, then switching to mainnet.
          </p>
        )}
      </div>
      ...
      <div className="mt-3 space-y-2 rounded-md border border-border/60 bg-background/40 p-2.5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {factoryAddress && (
              <>
                ...
                <a
                  href={explorerAddressUrl(explorerBase, factoryAddress)}
                  target="_blank"
                  rel="noreferrer"
          ...
          <div className="text-[0.65rem]">
            {probingFactory ? (
              <span className="inline-flex items-center gap-1 text-muted-foreground">
                <Loader2 className="size-3 animate-spin" />
                Checking factory on {targetNet.label}…
              </span>
            ) : factoryProbe?.ok ? (
              ...
              <span className="inline-flex items-center gap-1 text-destructive">
                ...
                {factoryProbe.reason === "no_code"
                  ? `No contract on ${targetNet.label} — redeploy the factory.`
        ...
        {factoryTx && (
          <p className="font-mono text-[0.65rem] text-muted-foreground">
            Factory tx:{" "}
            <a
              href={explorerTxUrl(explorerBase, factoryTx)}
              target="_blank"
              rel="noreferrer"
              className="text-primary hover:underline"
            >
      ...
      <div className="mt-3 space-y-2 rounded-md border border-border/60 bg-background/40 p-2.5">
        ...
        <p className="text-[0.65rem] text-muted-foreground">
          ...
          {registerGas ? (
            <>
              ...
              <span className="font-mono">{registerGas.toLocaleString()}</span> (formula-based; public RPCs can&apos;t
        ...
        {registerTx && (
          <p className="font-mono text-[0.65rem] text-muted-foreground">
            Register tx:{" "}
            <a
              href={explorerTxUrl(explorerBase, registerTx)}
              target="_blank"
              rel="noreferrer"
              className="text-primary hover:underline"
            >
        ...
        <div className="mt-3 space-y-2 rounded-md border border-emerald-500/40 bg-emerald-500/10 p-2.5">
          ...
          <div className="grid gap-1 text-[0.65rem] text-muted-foreground">
            <p>
              Token:{" "}
              <a
                href={explorerAddressUrl(explorerBase, launched.tokenAddress)}
                target="_blank"
                rel="noreferrer"
            ...
            <p>
              Vault:{" "}
              <a
                href={explorerAddressUrl(explorerBase, launched.vaultAddress)}
                target="_blank"
                rel="noreferrer"
                className="font-mono text-primary hover:underline"
              >
            ...
            <p>
              Launch tx:{" "}
              <a
                href={explorerTxUrl(explorerBase, launched.txHash)}
                target="_blank"
                rel="noreferrer"
                className="font-mono text-primary hover:underline"
              >
        ...
        <div className="mt-3 space-y-2 rounded-md border border-border/60 bg-background/40 p-2.5">
          ...
          {isMainnetTarget && (
            <div className="space-y-2 rounded-md border border-amber-500/40 bg-amber-500/5 p-2.5">
              <p className="text-[0.7rem] font-medium text-amber-200">Mainnet confirmation required</p>
              <p className="text-[0.65rem] text-muted-foreground">
                This spends real BNB. Buy/sell tax and token metadata are locked forever after launch. Advisory
                pipeline checks are not a professional security audit.
              </p>
              <label className="flex items-start gap-2 text-[0.65rem] text-foreground">
                <input
                  type="checkbox"
                  className="mt-0.5"
                  checked={mainnetAckRealFunds}
                  disabled={busy}
                  onChange={(e) => setMainnetAckRealFunds(e.target.checked)}
                />
                I understand this uses real BNB and the launch is irreversible.
              </label>
              <label className="flex items-start gap-2 text-[0.65rem] text-foreground">
                <input
                  type="checkbox"
                  className="mt-0.5"
                  checked={mainnetAckReviewed}
                  disabled={busy}
                  onChange={(e) => setMainnetAckReviewed(e.target.checked)}
                />
                I reviewed the plan/code and understand the fork-test results.
              </label>
              <label className="flex items-start gap-2 text-[0.65rem] text-foreground">
                <input
                  type="checkbox"
                  className="mt-0.5"
                  checked={mainnetAckNotAudit}
                  disabled={busy}
                  onChange={(e) => setMainnetAckNotAudit(e.target.checked)}
                />
                I accept that automated reviews are advisory, not a full audit.
              </label>
              <label className="block space-y-1 text-[0.65rem]">
                <span className="text-muted-foreground">
                  Type your token symbol to confirm
                  {tokenSymbol.trim() ? ` (${tokenSymbol.trim().toUpperCase()})` : ""}
                </span>
                <Input
                  value={mainnetConfirmText}
                  disabled={busy}
                  onChange={(e) => setMainnetConfirmText(e.target.value)}
                  placeholder={tokenSymbol.trim().toUpperCase() || "SYMBOL"}
                  className="w-40 text-[0.65rem]"
                />
              </label>
              {!mainnetGateOk && (
                <p className="text-[0.6rem] text-muted-foreground">
                  Tick all boxes and type the symbol exactly to enable mainnet launch.
                </p>
              )}
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              {phase === "launching" ? "Launching…" : isMainnetTarget ? "Launch on mainnet" : "Launch token"}
            </Button>
          </div>
          ...
          {/* Buy/sell tax — Flap docs: 100 = 1%, max 1000 = 10%. Defaults to 5%. */}
          <div className="space-y-1.5 rounded-md border border-border/40 bg-background/30 p-2">
            <p className="text-[0.65rem] font-medium text-foreground">Trading tax (you choose)</p>
            <div className="flex flex-wrap items-end gap-3">
              <label className="space-y-1">
                <span className="block text-[0.6rem] text-muted-foreground">Buy tax %</span>
                <Input
                  type="number"
                  min={1}
                  max={10}
                  ...
                  value={buyTaxPercent}
                  disabled={busy}
                  onChange={(e) => setBuyTaxPercent(e.target.value)}
                  className="w-24 text-[0.65rem]"
                />
              </label>
              <label className="space-y-1">
                <span className="block text-[0.6rem] text-muted-foreground">Sell tax %</span>
                <Input
                  type="number"
                  min={1}
                  max={10}
                  step={0.01}
                  value={sellTaxPercent}
                  disabled={busy}
                  onChange={(e) => setSellTaxPercent(e.target.value)}
                  className="w-24 text-[0.65rem]"
                />
              </label>
              <span className="pb-1.5 text-[0.6rem] text-muted-foreground">
                {taxInvalid
                  ? `Each tax must be between 1% and ${MAX_PORTAL_TAX_BPS / 100}%. Locked after launch.`
                  : `${buyTaxBpsPreview}/${sellTaxBpsPreview} bps · locked after launch`}
              </span>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-[0.6rem] text-muted-foreground">
              Initial buy at launch — this amount of {isMainnetTarget ? "BNB" : "tBNB"} is sent with the launch
              tx (plus gas).
            </span>
          </div>
            ...
            <p className="text-[0.65rem] text-muted-foreground">
              {!launchReady
                ...
                : !isRegistered
                  ...
                  : launchPayloadIssue?.message ?? `Connect wallet and switch to ${targetNet.label}.`}
            </p>
          )}
          ...
          <p className="text-[0.65rem] text-muted-foreground">
            After launch, your token page opens on{" "}
            <button type="button" onClick={openFlap} className="text-primary hover:underline">
              {flapBase.replace(/^https?:\/\//, "")}
            </button>{" "}
            at{" "}
            <span className="font-mono">
              /{isMainnetTarget ? "bnb" : "bnb-testnet"}/&lt;address&gt;
            </span>
            .
          </p>
        </div>
      ...
      {isConnected && !onTargetNetwork && (
        <Button
          ...
          variant="outline"
          disabled={busy}
          onClick={() => void ensureTargetNetwork()}
          className="mt-2"
        >
      ...
      <details className="mt-2 text-[0.65rem] text-muted-foreground">
        ...
        <div className="mt-1 grid gap-0.5 rounded border border-border/60 bg-background/60 p-1.5 font-mono">
          ...
          <span>
            chain: {targetChainId} ({targetNet.label}; wallet on {isConnected ? chainId : "not connected"})
          </span>
      ...
      <details className="mt-2 text-[0.65rem] text-muted-foreground">
        ...
        <div className="mt-1 grid gap-0.5 rounded border border-border/60 bg-background/60 p-1.5 font-mono">
          ...
          <span>portal: {launchPortalForChain(targetChainId)}</span>
          <span>
            chain: {targetChainId} ({targetNet.label}; wallet on {isConnected ? chainId : "not connected"})
          </span>
          <span>wallet: {address ?? "not connected"}</span>
          ...
          <span>
            buy/sell tax:{" "}
            {buyTaxBpsPreview ?? "?"}/{sellTaxBpsPreview ?? "?"} bps ({buyTaxPercent.trim() || "?"}% /{" "}
            {sellTaxPercent.trim() || "?"}%)
          </span>
          <span>factory: {factoryAddress ?? "not deployed"}</span>
